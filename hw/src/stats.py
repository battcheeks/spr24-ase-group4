...
class SAMPLE:
    ...
    def bar(self, num, fmt="%8.3f", word="%10s", width=50):
        out = [' '] * width
        def pos(x): return int(
            width * (x - self.lo) / (self.hi - self.lo + 1E-30))
        has = num.ok().has
        [a, b, c, d, e] = [has[int(len(has)*x)]
                           for x in [0.5, 0.25, 0.5, 0.75, 0.95]]
...
def _bootstrap(y0, z0, confidence=.05, Experiments=512,):
    ...
    def obs(x, y): return abs(x.mu - y.mu) / \
        ((x.sd**2 / x.n + y.sd**2 / y.n)**.5 + 1E-30)
